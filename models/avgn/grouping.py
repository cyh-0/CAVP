...
class GroupingBlock(nn.Module):
    ...
    def forward(self, x, group_tokens, return_attn=False):
        """
        Args:
            x (torch.Tensor): image tokens, [B, HW, C]
            group_tokens (torch.Tensor): group tokens, [B, S_1, C]
        ...
        new_x, attn_dict = self.assign(x, projected_group_tokens, return_attn=return_attn)
...
class GroupingBlock_Han(nn.Module):
    """Grouping Block to group similar segments together.
...
class ModalityTrans(nn.Module):
    ...
    def __init__(self,
        ...
        # han encoder
        if use_han:
            # self.han_encoder = Encoder(HANLayer(d_model=dim, nhead=8, dim_feedforward=dim), num_layers=1)
    ...
    def forward(self, x, group_token=None, x_other=None, return_attn=False):
        """
        Args:
            ...
            return_attn (bool): whether to return attention maps
        """
        group_token = group_token.expand(x.size(0), -1, -1)
        ...
        x_attn, group_token = self.split_x(cat_x)

        # NOTE: han encoder
        if self.han_encoder is not None and x_other is not None:
            x_attn, _ = self.han_encoder(x_attn, x_other)
...
class UniModalityTrans(nn.Module):
    ...
    def __init__(self,
        ...
        super(UniModalityTrans, self).__init__()
