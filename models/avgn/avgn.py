...
from models.avgn.grouping import ModalityTrans
...
class MMFusion(nn.Module):
    def __init__(
        ...
        attn_assign = 'soft'
        unimodal_hard_assignment = True if attn_assign == 'hard' else False
        dim = 304
        depth_vis = 3
        depth_aud = 3
        self.av_token = nn.Parameter(torch.zeros(num_classes, dim))
        self.audio_encoder = ModalityTrans(
                    ...
                    num_group_tokens=num_classes,
                    num_output_groups=num_classes,
    ...
    def forward_train(self, image, audio=None, shuffle_info=None, ow_flag=False):
        ...
        fea_v = rearrange(fea_v, "b c h w -> b (h w) c")
        fea_a = fea_a.unsqueeze(1)
        fea_v, _, _ = self.visual_encoder(fea_v, self.av_token, return_attn=True)
        fea_a, _, _ = self.audio_encoder(fea_a, self.av_token, return_attn=True)
