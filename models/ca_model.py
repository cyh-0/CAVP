...
class MMFusion(nn.Module):
    def __init__(
        ...
        logger.info(f"CURRENT FUSION METHOD IS {self.fusion_method.upper()}")
        if self.fusion_method == "attention":
            ...
            self.visual_projector = ProjectionHead(self.latent_dim*2, self.latent_dim)
    ...
    def forward_concat(self, fea_v, fea_a):
        fea_a = (fea_a.unsqueeze(-1).unsqueeze(-1)).repeat(1, 1, fea_v.shape[-2], fea_v.shape[-1])
        f_fuse = torch.cat((fea_v, fea_a), dim=1)
        return self.visual_projector(f_fuse)

    def forward_add(self, fea_v, fea_a):
        fea_a = (fea_a.unsqueeze(-1).unsqueeze(-1)).repeat(1, 1, fea_v.shape[-2], fea_v.shape[-1])
        f_fuse = torch.add(fea_v, fea_a)
        return self.visual_projector(f_fuse)
    ...
    def forward_train(self, image, audio=None, shuffle_info=None, ow_flag=False):
        ...
        fea_a = self.audio_backbone(audio)
        if self.fusion_method == "attention":
            out_fusion = self.forward_fusion(fea_v, fea_a)
        elif self.fusion_method == "concat":
            out_fusion = self.forward_concat(fea_v, fea_a)
        elif self.fusion_method == "add":
            out_fusion = self.forward_add(fea_v, fea_a)
        else:
            raise ValueError
