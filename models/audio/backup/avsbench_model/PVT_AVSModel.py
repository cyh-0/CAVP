...
from models.audio.backup.avsbench_model.pvt import pvt_v2_b5
from models.audio.backup.avsbench_model.TPAVI import TPAVIModule
import pdb
...
class Pred_endecoder(nn.Module):
    ...
    def __init__(
        ...
        self.upsample025 = nn.Upsample(
            scale_factor=0.25, mode="bilinear", align_corners=True
        )

        self.conv4 = self._make_pred_layer(
            Classifier_Module, [3, 6, 12, 18], [3, 6, 12, 18], channel, self.vis_dim[3]
        )
        self.conv3 = self._make_pred_layer(
            Classifier_Module, [3, 6, 12, 18], [3, 6, 12, 18], channel, self.vis_dim[2]
        )
        self.conv2 = self._make_pred_layer(
            Classifier_Module, [3, 6, 12, 18], [3, 6, 12, 18], channel, self.vis_dim[1]
        )
        self.conv1 = self._make_pred_layer(
            Classifier_Module, [3, 6, 12, 18], [3, 6, 12, 18], channel, self.vis_dim[0]
        )
    ...
    def forward(self, x, audio_feature=None):
        ...
        if len(self.tpavi_stages) > 0:
            ...
            for i in self.tpavi_stages:
                ...
                feature_map_list[
                    i
                ] = conv_feat  # update features of stage-i which conduct non-local
    ...
    def initialize_pvt_weights(
        ...
        state_dict = {
            k: v
            for k, v in pretrained_state_dicts.items()
            if k in pvt_model_dict.keys()
        }
