...
class TPAVIModule(nn.Module):
    ...
    """
    tmp = rearrange(f_div_C.clone().squeeze(), '(t h w) c -> t c h w', h=28, w=28, t=5)
    a = tmp.view(5, 5, 56*56, 56, 56)[0,0,:,:]
    torchvision.utils.save_image(a.sum(0)[None,None], "atten_main_2.png")

    torchvision.utils.save_image(tmp.sum(dim=1).unsqueeze(1), "atten_main.png")
    """
