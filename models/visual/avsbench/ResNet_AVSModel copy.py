...
from models.visual.avsbench.resnet import B2_ResNet, B101_ResNet
...
class Pred_endecoder(nn.Module):
    ...
    def __init__(
        ...
        # self.resnet = B101_ResNet()
        self.resnet = Backbone(
            back_bone=config.visual_backbone,
            norm_layer=nn.BatchNorm2d,
            pretrained_model=config.visual_backbone_pretrain_path,
            last_three_dilation_stride=config.last_three_dilation_stride,
        )

        self.conv4 = self._make_pred_layer(
            Classifier_Module, [3, 6, 23, 3], [3, 6, 23, 3], channel, 2048
        )
        self.conv3 = self._make_pred_layer(
            Classifier_Module, [3, 6, 23, 3], [3, 6, 23, 3], channel, 1024
        )
        self.conv2 = self._make_pred_layer(
            Classifier_Module, [3, 6, 23, 3], [3, 6, 23, 3], channel, 512
        )
        self.conv1 = self._make_pred_layer(
            Classifier_Module, [3, 6, 23, 3], [3, 6, 23, 3], channel, 256
        )
    ...
    def forward(self, x, audio_feature=None):
        ...
        # x = self.resnet.backbone.conv1(x)
        # x = self.resnet.backbone.bn1(x)
        # x = self.resnet.backbone.relu(x)
        # x = self.resnet.backbone.maxpool(x)
        # x1 = self.resnet.backbone.layer1(x)  # BF x 256  x 56 x 56
        # x2 = self.resnet.backbone.layer2(x1)  # BF x 512  x 28 x 28
        # x3 = self.resnet.backbone.layer3(x2)  # BF x 1024 x 14 x 14
        # x4 = self.resnet.backbone.layer4(x3)  # BF x 2048 x  7 x  7
        ...
        # print(pred.shape)
        pred = F.interpolate(
            pred, size=input_shape, mode="bilinear", align_corners=False
        )

        return pred, feature_map_list, a_fea_list

    def initialize_weights_50(self):
    ...
    def initialize_weights_101(self):
        res50 = models.resnet101(pretrained=False)
        resnet50_dict = torch.load(self.cfg.TRAIN.PRETRAINED_RESNET50_PATH)
        res50.load_state_dict(resnet50_dict)
        pretrained_dict = res50.state_dict()
        # print(pretrained_dict.keys())
        ...
        self.resnet.load_state_dict(all_params)
        print(
            f"==> Load pretrained ResNet101 parameters from {self.cfg.TRAIN.PRETRAINED_RESNET101_PATH}"
        )
