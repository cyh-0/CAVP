...
class BasicBlock(nn.Module):
    expansion = 1
    def __init__(self, inplanes, planes, stride=1, norm_layer=None, dilation=1,
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, norm_layer=None, bn_eps=1e-5, bn_momentum=0.1,
                 downsample=None, inplace=True, dilation=1):
...
class ResNet(nn.Module):
    def __init__(self, block, layers, norm_layer=nn.BatchNorm2d, groups=1, bn_eps=1e-5,
    ...
    def _make_layer(self, block, norm_layer, planes, blocks, inplace=True,
                    stride=1, bn_eps=1e-5, bn_momentum=0.1, dilate=False):
        ...
        layers.append(block(self.inplanes, planes, stride, norm_layer, bn_eps,
                            bn_momentum, downsample, inplace, dilation=previous_dilation))

        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, norm_layer=norm_layer, bn_eps=bn_eps,
                                bn_momentum=bn_momentum, downsample=None, inplace=True, dilation=self.dilation))
...
def resnet18(pretrained_model=None, **kwargs):
    ...
    if pretrained_model is not None:
        ...
        print(f'==> Load pretrained ResNet18 parameters from {pretrained_model}')
...
def resnet50(pretrained_model=None, **kwargs):
    model = ResNet(Bottleneck, [3, 4, 6, 3], **kwargs)
    pretrained_model = "ckpts/pretrained/resnet50.pth"
    if pretrained_model is not None:
        model = load_model(model, pretrained_model)
        print(f'==> Load pretrained ResNet50 parameters from {pretrained_model}')
    return model


def resnet101(pretrained_model=None, **kwargs):
    model = ResNet(Bottleneck, [3, 4, 23, 3], **kwargs)
    pretrained_model = "ckpts/pretrained/resnet101.pth"
    if pretrained_model is not None:
        model = load_model(model, pretrained_model)
        print(f'==> Load pretrained ResNet101 parameters from {pretrained_model}')
    return model
