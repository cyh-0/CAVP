...
class BASELINE:
    ...
    def train(
        ...
        for batch_idx, (image, waveform, pix_label, img_label, _) in enumerate(tbar):
            ...
            """
            weight, pseudo_target = torch.max(torch.softmax(pseudo_logits, dim=1), dim=1)
            # pseudo_target[weight < .75] = 255
            loss_pseudo = torch.nn.functional.cross_entropy(output, pseudo_target, ignore_index=255,
                                                            reduction="none") * weight
            loss_cam = torch.nn.functional.cross_entropy(output, pix_label.squeeze().long(), ignore_index=255,
                                                         reduction="none") # * (1.-weight)
            """
            output = model_v_(image)
    ...
    def validation(self, model_v_, model_a_, epoch, test_loader, status="val"):
        ...
        if self.local_rank <= 0:
            ...
            self.vis_tool.upload_metrics(
                {"epoch": epoch, "miou": v_miou, "acc": v_acc, "fdr": v_fd, "f1": v_f1}, epoch
            )

        return

    @torch.no_grad()
    def validation(self, model_v_, model_a_, epoch, test_loader, local_rank, status="val"):
        model_v_.eval()
        model_a_.eval()
        target_list, audio_fea_list, c_list = [], [], []
        miou_measure = MIoU(
