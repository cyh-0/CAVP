...
class BASELINE:
    ...
    def lr_step(self, optimizer_v_, current_lr, use_baseline=False):
        ...
        # CONV+ATTEN
        for _, opt_group in enumerate(optimizer_v_.param_groups[2:18]):
            opt_group["lr"] = current_lr
        # Segmentation
        for _, opt_group in enumerate(optimizer_v_.param_groups[18:]):
            opt_group["lr"] = current_lr * 10.0

        lr_bkb = optimizer_v_.param_groups[0]["lr"]
        lr_attn = optimizer_v_.param_groups[2]["lr"]
        lr_seg = optimizer_v_.param_groups[18]["lr"]
    ...
    def train(
        ...
        for batch_idx, (image, waveform, pix_label, img_label, _) in enumerate(tbar):
            ...
            with torch.no_grad():
                audio_feature = model_a_(audio)  # [B*T, 128]

            output, visual_map_list, a_fea_list = model_v_(image, audio_feature)
            ctr_feature = torch.tensor([0.0], device=self.local_rank)
            l_ctr_av = torch.tensor([0.0], device=self.local_rank)
            ...
            del img_label, waveform

            current_lr = self.lr_scheduler.get_lr(
                cur_iter=batch_idx + loader_len * epoch
            )

            lr_bkb, lr_attn, lr_seg = self.lr_step(
                optimizer_v_, current_lr, use_baseline=self.args.use_baseline
            )
            ...
            del image, output, pix_label, loss
        return
    ...
    def validation(self, model_v_, model_a_, epoch, test_loader, status="val"):
        ...
        for batch_idx, (image, waveform, pix_label, img_label, name) in enumerate(tbar):
            ...
            with torch.no_grad():
                audio_feature = model_a_.module(audio)  # [B*T, 128]


            image = TF.resize(image, (224, 224), interpolation=transforms.InterpolationMode.BICUBIC)
            pix_label = TF.resize(pix_label, (224, 224), interpolation=transforms.InterpolationMode.NEAREST)


            avl_map_logits, _, _ = model_v_.module(image, audio_feature)
