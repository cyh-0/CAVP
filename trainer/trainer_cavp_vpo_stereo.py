...
class CAVP_TRAINER:
    ...
    def overwrite_miss_match(self, if_match, shuffle_img_label, img_label):
        ...
        ms_idx = torch.where(source_count != 1)[0] # multi-source idx
        bg_only_idx = torch.where(img_label.sum(-1) == 1)[0]
        ...
        """ Filter Background Only Sample"""
        match_idx = torch.isin(change_idx, bg_only_idx)
        change_idx = change_idx[~match_idx]

        """ Incase we drop idx under single source setup """
        if self.args.setup != "vpo_ms" and self.args.setup != "vpo_msmi":
    ...
    def train(
        ...
        for batch_idx, (image, waveform, pix_label, img_label, case_id) in enumerate(tbar):
            ...
            output_cat, ctr_feature_cat, _ = model_v_(image, audio, shuffle_info, ow_flag, audio_func=True)
            B = image.shape[0]
    ...
    def validation(self, model_v_, model_a_, epoch, test_loader, status="val"):
        ...
        if self.local_rank <= 0:
            ...
            self.vis_tool.upload_metrics(
                {"miou": v_miou, "acc": v_acc, "fdr": v_fd, "f1": v_f1, "f_0.3": v_f03},
                epoch
            )
