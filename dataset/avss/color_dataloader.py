...
class V2Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # df_one_video = self.df_split[self.df_split.uid == "D9LbDE-1FLQ_172000_182000"].iloc[0]
        ...
        # wave_, rate_ = torchaudio.load(audio_path)
        # wave_ = torchaudio.transforms.Resample(rate_, 16000)(wave_)
        # label_set = torch.unique(labels_tensor.view(labels_tensor.shape[0],-1), dim=-1)

        class_label = [F.one_hot(torch.unique(labels_tensor[i]), num_classes=self.num_classes).sum(0) for i in range(labels_tensor.shape[0])]
        class_label = torch.stack(class_label, dim=0)

        if self.split == "train":
            num_gt = gt_temporal_mask_flag.sum().long().item()
            idx_f = random.randint(0,num_gt-1)
            imgs_tensor = imgs_tensor[idx_f,:,:]
            # class_label = torch.sum(class_label, dim=1)
            labels_tensor = labels_tensor[idx_f].squeeze()
            class_label = class_label[idx_f]
        else:
            return imgs_tensor, audio_path, labels_tensor, class_label, \
             vid_temporal_mask_flag, gt_temporal_mask_flag, mask_num, video_name
