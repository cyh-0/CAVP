...
from dataset.vpo_stereo.multi_source.audio.audio_dataset import AudioDataset
from dataset.vpo_stereo.multi_source.visual.visual_dataset import VisualDataset
from dataset.vpo_stereo.multi_source.visual.visual_aug import VisualAugmentation
...
class AudioVisualDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.mode == "train":
            image, label, category, file_id, chosen_flip = self.dataset_v.__getitem__(idx)
            audio_waveform = self.dataset_a.__getitem__(idx, flip=chosen_flip)
            return image, audio_waveform, label, category, file_id
        else:
            image, label, category, file_id, _ = self.dataset_v.__getitem__(idx)
